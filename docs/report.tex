...
it copes with different images. 
...
the problem, I broke it up into several scripts and functions, each with a single
clearly defined purpose, this helped to ensure clarity and ease of reuse.  
...
The MATLAB programs that I had written took a very long time to complete,
executions for 50 divisions were taking up to two minutes. At
first I though that this was due to the time complexity, but after a quick
calculation I realised that it should be much quicker. I realised that making
function calls in MATLAB is actually very expensive. Inlining all of my
functions made the code run much faster, my code went from taking an average of
thirty seconds to complete to just nine. I believe this behavior is a side
effect of the way that MATLAB is interpreted. 

I felt that it would still be possible to make my algorithm faster. I targeted
the deepest loop in my program as this is where the program was spending the
vast majority of its time. I found that MATLAB's Matrix and Vector operations
were actually quite slow. By replacing these operations with normal arithmetic
where possible, and moving all calculations into the outermost loop possible, I
was able to reduce the execution time to under one second.
...
The code that I wrote for the optimisation is very unreadable and doesn't adhere
to good programming practice. I believe that had I written this program in a
compiled language, I wouldn't have had these problems.
...
\begin{thebibliography}{9}
	...
	\bibitem{heartImg}
		...
		gustty,\\
		http://www.flickr.com/photos/gustty/126606794/
...
The initial open contour used to calculate the end points for the closed
contour.
